---
title: "Lab4a - Sampling Distributions"
...
1.  Describe this population distribution. This distribution of the population is approx. symetric and it has a strong skew, there is not visible outliers. For the  total population of interest the average ground living area of the house in square feet is 1499.690 and the standard deviation for the living area of the house is 505.5088. Becuause this it the population sample. Because the sampling population is unknow this distribution doesn't comply with the first condition that the sample should lees than 10% of the total population.
```{r}
tarea_mean <- mean(area)
tarea_sd <- sd(area)
```
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    The distribution for the sample population is bimodal or multimodal with an        unclear skewed to the right.
    
   *Another method to compare the outcome is making a Q-Q plot for both, the           sample with 50 observations and the population size.(please see below)*
    
    The histogram and normal probability plots for the sample with 50 observations     is too small to clearly see a clear structure in the histogram. The normal         probaility plot has many observations that deviate noticeably from the             line. While the distribution of the population looks more normal but it has        also  remarkable deviations.
...
x <- 700:2500
y <- dnorm(x = x, mean = areamean, sd = areasd)
lines(x = x, y = y, col = "blue")
```
```{r}
#Sample1's Normal probability plot
qqnorm(samp1)
qqline(samp1)
#Population's Normal probabilty plot
qqnorm(area)
qqline(area)
```
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    *R1/: The mean of sample 2 is greater than the mean of sample 1* 
    *R2/: If the sample mean doesn't change much from one sample to another then        that estimate of the population mean is probably very accurate while if the        sample from one sample to another varies widely the estimate of the population      mean should not be accurate.*
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    *R1:/There are 5000 elements in `sample_means50`*
    *R2:/The distribution for the sample population is unimodal and approx.             symetric because, the sample mean is 1468.2 which falls into the true              population mean which is 1499.69. It is also center exactly at the true            population mean:1499.69*
    *R3:/ The distribution of 50000 sample means of size 50 would be exactly the        same, the difference between the means of both, the 5000 sample means:1500.146
     and the 50000 sample means:1499.328 is minimal. 
    
```{r}
sample3_means50 <- rep(NA, 50000)

for(i in 1:50000){
   samp3 <- sample(area, 50)
   sample3_means50[i] <- mean(samp3)
   }

hist(sample3_means50, breaks = 25)
```
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    *R1:/ There are 100 elements in the vector `sample_means_small`*
    *R2:/ Each element represents 100 sample of the means.*
...
6.  When the sample size is larger, what happens to the center?  What about the spread?
  ...
  *R1:/ The center is very closed to the population mean.*
  *R2:/ The spread of a distribution refers to the variability of the data. Let's          take a look of the samples;*
        *a) sample_means10: the observations cover a range of 1307.2 to 1695.02*
        *b) sample_means50: the observations cover a range of 1263.44 to 1781.9*
        *c) sample_means100: the observations cover a range of 1450.135 to                  1599.327*
        
___Analyzing the variability of the data and taking those ranges with min() and     max() fuction for each vector; plots a) and b) cover a wide range, the spread is larger () , while plot c)covers a small range, being streched, the spread is smaller (Less Spread).___
...
*R:/ The Sample mean = $200833 is our point of estimate of the population for the ramdon sample.
...
The distribution is symetric, it is centered at the true average home price of the population mean, the spread of the distribution indicates how much variability is involved by sampling 50 home sales price. 
...
for(i in 1:5000){
  price_samp <- sample(price, 50)
  sample_means50[i] <- mean(price_samp)
   }
...
###The population mean for the sampling size of 50 home sales price 
```{r}
price_mean <- mean(sample_means50)
price_mean
```
...
for(i in 1:5000){
   price_samp <- sample(price, 150)
   sample_means150[i] <- mean(price_samp)
   }
...
###Comparison between the sample distribution of 50 and 150 home sales price

```{r plot-samp-price, eval=TRUE, tidy = FALSE}
par(mfrow = c(2, 1))
...
Both distributions are symetric and unimodal

R1.The center is almost the same in both plots, the population mean.
R2:The spread of a distribution refers to the variability of the data. Let's take a look of the samples;
a) sample_means50: the observations cover a range of 142261.6 to 235461.2
b) sample_means150: the observations cover a range of 160189.2 to 205438.1
...
Analizing the variability of the data and taking those ranges with min() and max() function for each vector; plot of 50 samples is more spread while plot of 150 samples is less spread, the distribution is squeezed when the sample means increase, hence, the sample size of 150 samples of the mean has a smaller          spread.
